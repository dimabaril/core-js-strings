...
function concatenateStrings(value1, value2) {
  // return value1.concat(value2);  // which is better?
...
function removeFirstOccurrences(str, value) {
  ...
  const startSlice = str.indexOf(value);
  const endSlice = startSlice + value.length;
...
function removeLastOccurrences(str, value) {
  if (!str.includes(value)) return str;
  const startSlice = str.lastIndexOf(value);
  const endSlice = startSlice + value.length;
  return str.slice(0, startSlice) + str.slice(endSlice);
}
...
function formatTime(minutes, seconds) {
  const twoNumMin = minutes.toString().padStart(2, '0');
  const twoNumSec = seconds.toString().padStart(2, '0');
  return `${twoNumMin}:${twoNumSec}`;
}
...
function countVowels(str) {
  ...
  return str.split('').filter((curr) => vowels.includes(curr)).length;
}
...
function isPalindrome(str) {
  const regex = /[,\s!?]/g;
  const modifiedStr = str.replaceAll(regex, '').toLowerCase();
  return modifiedStr.split('').reverse().join('') === modifiedStr;
}
...
function invertCase(str) {
  return str
    .split('')
    .map((curr) => {
      if (curr === curr.toUpperCase()) return curr.toLowerCase();
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((curr) => {
      if (!alphabet.includes(curr)) return curr;
      return rot13[alphabet.indexOf(curr)];
    })
    .join('');
}
